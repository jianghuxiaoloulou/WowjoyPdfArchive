...
func (c *Client) Read() {
	defer func() {
		...
		global.Logger.Debug("client ", c.Id, " disconnect")
		// log.Printf("client [%s] disconnect", c.Id)
		if err := c.Socket.Close(); err != nil {
			global.Logger.Error("client: ", c.Id, " disconnect err: ", err)
	...
	for {
		...
		global.Logger.Debug("client: ", c.Id, " receive message: ", string(message))
...
func (c *Client) Write() {
	...
	for {
		...
		case message, ok := <-c.Message:
			...
			global.Logger.Debug("client: ", c.Id, " write message:", string(message))

			// err := c.Socket.WriteJSON(message)
			err := c.Socket.WriteMessage(websocket.TextMessage, message)
			// err := c.Socket.WriteMessage(websocket.BinaryMessage, message)
			if err != nil {
				global.Logger.Error("client: ", c.Id, " writemessage err:", err)
			}
		}
	}
}
...
func (manager *Manager) Start() {
	global.Logger.Debug("websocket manage start")
	for {
		select {
		// 注册
		case client := <-manager.Register:
			global.Logger.Debug("client: ", client.Id, " connect")
			manager.Lock.Lock()
		...
		case client := <-manager.UnRegister:
			global.Logger.Debug("unregister client:", client.Id)
			manager.Lock.Lock()
...
func (manager *Manager) WsClient(ctx *gin.Context) {
	...
	if err != nil {
		global.Logger.Error("websocket connect error: ", ctx.Param("uidenc"))
		return
	}
	...
	// // manager.Send(client.Id, client.Group, []byte("你好"))
}
...
func TestSendData() {
	for {
		time.Sleep(time.Second * 25)
		WebsocketManager.Send("1234567", "PDF", []byte("你好"))
	}
}
